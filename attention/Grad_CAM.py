...
# 保存模型参数
# https://blog.csdn.net/weixin_44575717/article/details/124079563
...
if __name__ =="__main__":
    ...
    # model.load_state_dict((torch.load("vgg16.pth")))  # 加载
    ...
    img = cv2.imread('images/0_original.png')  # 读取图像
    ...
    # ——————————————————————训练——————————————————————————————————————
    # log = open("para.txt", mode="a", encoding="utf-8")
    # # 定义模型
    # model = models.vgg16(pretrained=False)
    # model.cuda()
    # for name, param in model.named_parameters():
    #     print(name, '      ', param.size(),file=log)
    # log.close()
    # # summary(model=model, input_size=(3, 224, 224),batch_size=1, device="cuda")
    #
    # img = cv2.imread(r"images/0_original.jpg")
    # img_1 = cv2.resize(img, (256, 256))
    # # print(img_1.shape)
    # x = torch.tensor(img_1).float()
    # x = x.permute(2, 0, 1)  # add
    # x = x.unsqueeze(0)
    # print(x.shape)
    # y = torch.ones(1)
    # y = y.to(torch.int64)
    ...
    # # 定义优化器
    # optimizer = optim.Adam(model.parameters(), lr=4e-4, weight_decay=1e-5)  # Adam梯度下降
    # # 训练
    ...
    #     optimizer.step()
    #
    # torch.save(model.state_dict(), "vgg16.pth")  # 将模型状态(参数)保存为字典形式
    # model.load_state_dict((torch.load("vgg16.pth"))) # 加载