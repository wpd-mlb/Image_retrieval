...
# patch 384 512 576 640
def load_arg():
    parser = ArgumentParser(description="Convert JPG file to NPY for acceleration")
    parser.add_argument("-root_dir", default='F:\\PycharmProjects_\\Graduation\\features\\data', type=str)
    parser.add_argument("-save_dir", default='F:\\PycharmProjects_\\Graduation\\features\\data', type=str)
    parser.add_argument("-patch", type=int, default=512)
    arg = parser.parse_args()
    return arg

# 保存 numpy
# with open('F:\\PycharmProjects_\\Graduation\\features\\data\\test_data_A' + 'num.txt', 'w') as f:
#     for i in range(len(image_new)):
#         f.write(str(image_new[i]) + '\n')
        ...
        for dir_0 in dir_0s:
            ...
            for image_name in image_names:
                ...
                if w > h:
                    ...
                    image_tmp = cv2.resize(image,(size_new, new_tmp))  # h w c
                    cv2.resize()
